...
class DatasetEmbedding(nn.Module):
    ...
    def __init__(self,tokenizer,llm_model,device):
        ...
        self.normalize_layers = _Normalize(self.feature_dimension, affine=False)

        # Reprogramming layer
        self.n_heads = 8
        self.d_ff = 32 # dimension of fcn
        self.d_llm = 4096 # dimension of llm model,LLama7b:4096; GPT2-small:768; BERT-base:768
        self.reprogramming_layer = _ReprogrammingLayer(self.d_model, self.n_heads, self.d_ff, self.d_llm) 
    ...
    def dataset_embedding(self,prompt,ts):
        ...
        dataset_concat_embedding = torch.cat([prompt_embeddings, ts_embeddings], dim=1)
        return dataset_concat_embedding
    ...
    def __time_series_embedding(self,ts):
        ...
        embedded_ts_patch, n_vars = self.patch_embedding(ts.to(torch.float32)) # (B,N,T) -> (B * N, num_patches P, d_model dm)


        # -- Patch Reprogramming: 
        prototype_embeddings = self.vocab_mapping_to_prototype_layer(self.llm_embeddings_weight.permute(1, 0)).permute(1, 0) 
        reprogramming_enc_out = self.reprogramming_layer(embedded_ts_patch, prototype_embeddings, prototype_embeddings)
