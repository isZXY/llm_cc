...
from dataset_input_embedding_updated import StateEmbedding,ActionEmbedding,ReturnEmbedding,TimeEmbedding
...
class Model(nn.Module):
    def __init__(self, plm_path, device):
        ...
        self.modules_except_llm = nn.ModuleList([
            self.state_embedding_layer.embed_state1,self.state_embedding_layer.embed_state2,self.state_embedding_layer.embed_state3,self.state_embedding_layer.embed_state4,self.state_embedding_layer.embed_state5, self.output_projection, self.action_head, self.embed_ln,self.action_embedding.embed_action, self.return_embedding.embed_return,self.time_embedding.embed_timestep
        ])
    ...
    def forward(self,states, actions, returns, timesteps, labels,attention_mask=None):
                ...
                states_embedding_list = self.state_embedding_layer(states,time_embeddings)
                ...
                for i in range(returns_embeddings.shape[1]):
                    stacked_input = torch.cat((returns_embeddings[:, i:i + 1], states_embedding_list[0][:, i:i + 1], states_embedding_list[1][:, i:i + 1], states_embedding_list[2][:, i:i + 1],states_embedding_list[3][:, i:i + 1],states_embedding_list[4][:, i:i + 1],action_embeddings[:, i:i + 1]), dim=1)
                    stacked_inputs.append(stacked_input)
                    action_embed_positions[i] = (i + 1) * (2 + 5 * 1)
